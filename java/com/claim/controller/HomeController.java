...
//				int[] enrollmentMonthList = {10,24,6,13,7,34};
//				model.addAttribute("enrollmentMonthList", enrollmentMonthList);
				model.addAttribute("enrollmentAgencyList", enrollmentService.getPieChartList());
				model.addAttribute("enrollmentMonthList", enrollmentService.getLineChartList());
				return "adminPortal";
			}
			else{
				return "userPortal";
	...
	//Controller to get invoice email sent
	@RequestMapping(value="sendInvoiceEmail", method=RequestMethod.GET)
	public String sendInvoiceEmail(Model model){
		sendEmail.sendInvoice("ek1074@gmail.com", "State Training Center Invoice", InvoiceTemplate.buildTemplate());
		return viewOutstanding(model);
	}
}
