...
public class EnrollmentService {
	...
	private static final String pieChartQuery="select acronym, count(enrollmentID) as numEnrollments from enrollments E INNER JOIN users U ON E.userID=U.userID RIGHT JOIN agency A ON U.agencyID=A.agencyID group by agencyName";
	private static final String lineChartQuery="SELECT DATE_FORMAT(dateOffered, '%b') as dateMonth, COUNT(enrollmentID) as numEnrollments FROM enrollments E INNER JOIN courseOfferings as C on E.courseOfferingID=C.courseOfferingID GROUP BY month(dateOffered)";
...
//		}
	...
	@Transactional
	public List<Object> getLineChartList() {
		return entityManager.createNativeQuery(lineChartQuery, Object.class)
				.getResultList();
	}
	
	@Transactional
	public List<Object> getPieChartList() {
		return entityManager.createNativeQuery(pieChartQuery, Object.class)
				.getResultList();
	}
